...
export default function AdminIncidentSimulationPage() {
  ...
  const [selectedOrganizationId, setSelectedOrganizationId] =
    useState<string>("");
  const [stepsPanelOffset, setStepsPanelOffset] = useState(0);
  const contentBeforeCardRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const loadOrganizations = async () => {
      try {
        ...
        if (orgIdFromQuery) {
          setSelectedOrganizationId(orgIdFromQuery);
  ...
  }, [clerkLoaded, searchParams]);

  // Update client name when organization is selected (for Step1)
  useEffect(() => {
      ...
      if (selectedOrg) {
        setFormData(
          (prev) =>
            ...
            } as Partial<IncidentSimulationData>)
        );
      }
    }
  }, [selectedOrganizationId, organizations]);

  // Measure content height before Card to align steps panel
  useEffect(() => {
    const updateOffset = () => {
      if (contentBeforeCardRef.current && cardContainerRef.current) {
        const contentBeforeCard = contentBeforeCardRef.current;
        const cardContainer = cardContainerRef.current;

        // Get the top position of the Card container
        const cardTop = cardContainer.getBoundingClientRect().top;
        // Get the top position of the content area
        const contentTop = contentBeforeCard.getBoundingClientRect().top;

        // Calculate the offset (height of content before Card)
        const offset = cardTop - contentTop;
        setStepsPanelOffset(Math.max(0, offset));
      }
    };
    ...
    // Use ResizeObserver to update on content changes
    const resizeObserver = new ResizeObserver(() => {
      // Small delay to ensure DOM is updated
      setTimeout(updateOffset, 10);
    });

    if (contentBeforeCardRef.current) {
      resizeObserver.observe(contentBeforeCardRef.current);
    }
  ...
  }, [currentStep, incidentId, selectedOrganizationId]);
  ...
  const handleSubmit = async () => {
    if (!clerkUser || !selectedOrganizationId) {
    ...
    try {
      ...
      > = {
        ...
        initialDetails: {
          clientName: formData.step1?.clientName || selectedOrg?.name || "",
        ...
        technical: {
          ...
          compromisedAccounts: {
            ...
            // Only include accountNames if it has a value (Firestore doesn't allow undefined)
            ...(formData.step6?.compromisedAccounts?.accountNames && {
      ...
      // Redirect to admin incidents page
      setTimeout(() => {
  ...
  return (
    <OnboardingProvider>
      <div
        className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20 px-6 pb-6 pt-1"
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="hidden lg:flex flex-col gap-4 w-48 flex-shrink-0">
              {/* Spacer to align with Card container - dynamically measured */}
              <div style={{ height: `${stepsPanelOffset}px` }}></div>
            ...
            {/* Main Content Area */}
            <div className="flex-1 min-w-0" ref={contentBeforeCardRef}>
              {/* Organization Selection - Only show on first step */}
              {currentStep === 0 && (
                <Card className="mb-6">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Choose the organization for which you&apos;re creating
                      this incident simulation
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Select
