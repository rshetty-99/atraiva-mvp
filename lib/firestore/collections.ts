...
export const organizationService = new FirestoreService<Organization>(collections.organizations);
export const breachService = new FirestoreService<DataBreach>(collections.breaches);
export const complianceService = new FirestoreService<ComplianceCheck>(collections.complianceChecks);
export const documentService = new FirestoreService<Document>(collections.documents);
export const piiScanService = new FirestoreService<PIIScanResult>(collections.piiScans);
export const auditService = new FirestoreService<AuditLog>(collections.auditLogs);
...
export const auditQueries = {
  ...
  async getByOrganization(organizationId: string, limitCount: number = 100): Promise<AuditLog[]> {
    const q = query(
      collection(db, collections.auditLogs),
