...
export const complianceEvents = {
  ...
  piiScanCompleted: (piiFound: number) =>
    trackEvent("pii_scan_completed", { pii_count: piiFound }),
  breachReportGenerated: (reportType: string) =>
    trackEvent("breach_report_generated", { report_type: reportType }),
  complianceCheckRun: (regulation: string) =>
    trackEvent("compliance_check_run", { regulation }),
  ...
  invoiceGenerated: (amount: number) =>
    trackEvent("invoice_generated", { value: amount, currency: "USD" }),

  // Engagement events
  dashboardVisited: () => trackEvent("dashboard_visited"),
  featureExplored: (feature: string) =>
    trackEvent("feature_explored", { feature_name: feature }),
  ...
  supportTicketCreated: (category: string) =>
    trackEvent("support_ticket_created", { category }),
...
export const setUserProperties = (properties: Record<string, any>) => {
  if (analytics && typeof window !== "undefined") {
    ...
    Object.entries(properties).forEach(([key, value]) => {
      logEvent(analyticsInstance, "user_property_set", {
        property_name: key,
...
export const enhancedEvents = {
  ...
  formAbandoned: (formName: string) =>
    trackEvent("form_abandoned", { form_name: formName }),
